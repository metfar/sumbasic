...
## Installation note for 0.1.0a15

Version 0.1.0a15 deliberately uses a new package version after a reported `0.1.0a14` installation loaded an earlier cached interpreter that did not yet contain the PLAY runtime branch. Install the a15 archive with normal upgrade semantics; `--check` now validates command recognition rather than only loading source lines.
