...
sumBASIC does **not** insert new tokens into that historical block because doing so would shift every later ASC code. The companion file in `extras/asc_h-sumbasic-0.1.0a6.py` therefore appends the modern sumBASIC vocabulary after the existing table, beginning at index `2990` in the supplied version.
...
Version 0.1.0a6 adds no new language-token positions: `BEEP` and `SOUND` already have historical codes 566 and 615, and F5 is an IDE key binding rather than an ASC token.
