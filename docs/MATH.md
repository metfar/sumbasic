...
This document describes the mathematical surface implemented in `0.1.0a5`. It does not claim that every field of mathematics is already built in: matrix algebra, statistics and symbolic algebra are separate future layers. Real, integer, decimal and complex arithmetic below are executable now.
